 ...
 * Query filter object
 */
export interface LpFilter<MT extends object = AnyObject> {
  /**
 ...
 */
declare type FilterExcludingWhere<MT extends object = AnyObject> = Omit<LpFilter<MT>, "where">;
/**
 * TypeGuard for Filter
 * @param candidate
 */
declare function isFilter<MT extends object>(candidate: any): candidate is LpFilter<MT>;
/**
 ...
 * ```
 */
declare class WhereBuilder<MT extends object = AnyObject> {
  where: Where<MT>;
 ...
 * ```
 */
declare class FilterBuilder<MT extends object = AnyObject> {
  filter: LpFilter<MT>;
  constructor(f?: LpFilter<MT>);
  /**
   * Set `limit`
   ...
   */
  impose(constraint: LpFilter<MT> | Where<MT>): this;
  /**
   * Return the filter object
   */
  build(): LpFilter<MT>;
}
declare function filterTemplate(strings: TemplateStringsArray, ...keys: any[]): (ctx: AnyObject) => any;
export interface LpInclude {
  relation: string;
}
export interface LpWhere {
  and?: LpWhere[];
  or?: LpWhere[];
  [key: string]: any;
}

export interface LpCount {
  count: number;
}
