  ...
  return (
    <div className="post-card">
      <div className="blog-owner">
        <img src={blog.mAdmin?.mAvatar} className="avatar" alt="" />
        <span className="name">{blog.mAdmin?.mDisplayName}</span>
      </div>

      <a
        className="title"
        onClick={() => {
          router.push(`/blog/${blog.mId}`);
        }}
      >
        {blog.mTitle}
      </a>
      <span className="datetime">{formatDate(blog.mCreated ?? "")}</span>
      <img className="image-preview" src={blog.mImage} alt="dasd" />
